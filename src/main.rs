...
fn main() {
    ...
    wstr_dir(&mut items, root.to_path_buf()).unwrap();
    wstr_file(&mut items, root.join("f"), String::from("asd")).unwrap();
    wstr_file(&mut items, root.join("D"), String::from("asddddddd")).unwrap();

    get_file_infos(Path::new(&path))
        .iter()
        .for_each(|x| println!("{:?}", x));

    wstr_dir(&mut items, root.join("dir")).unwrap();
    wstr_file(&mut items, root.join("dir/x"), String::from("_____")).unwrap();
    get_file_infos(Path::new(&path))
        .iter()
        .for_each(|x| println!("{:?}", x));

    wstr_dir(&mut items, root.join("dir2")).unwrap();
    wstr_file(&mut items, root.join("dir2/x"), String::from("_____")).unwrap();
